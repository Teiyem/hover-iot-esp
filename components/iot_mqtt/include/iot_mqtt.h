...
private:
    ...
    esp_mqtt_client_handle_t _client;                                         /**< Handle to the mqtt client. */
    std::mutex _subscribe_mutex;                                              /**< Mutex for protecting access to the callbacks map. */
    std::mutex _cb_mutex;                                                     /**< Mutex for protecting access to the event callbacks map. */
    std::unordered_map<std::string, iot_mqtt_subscribe_cb_t> _subscriptions;  /**< Map of topic names to their corresponding callbacks. */
